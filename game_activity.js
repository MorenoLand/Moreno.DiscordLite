...
function getContentBounds(){var cr=document.querySelector('[class*="contentRegion"]');if(cr&&cr.isConnected){var r=cr.getBoundingClientRect();if(r.height>200&&r.width>200){return {left:Math.round(r.left),top:Math.round(r.top),right:Math.max(0,Math.round(window.innerWidth-r.right)),bottom:Math.max(0,Math.round(window.innerHeight-r.bottom))};}}var refLi=findSidebarActivityPrivacy();var sidebar=(refLi&&refLi.closest('[class*="sidebarRegion"],nav[class*="sidebar"],[class*="sidebar"]'))||document.querySelector('[class*="sidebarRegion"],nav[class*="sidebar"]');if(sidebar&&sidebar.isConnected){var sb=sidebar.getBoundingClientRect();if(sb.width>50&&sb.height>200){return {left:Math.round(sb.right),top:Math.round(sb.top),right:0,bottom:Math.max(0,Math.round(window.innerHeight-sb.bottom))};}}return {left:240,top:48,right:0,bottom:0};}
function position(panel){if(!panel)return;var b=getContentBounds();panel.style.setProperty('position','fixed','important');panel.style.setProperty('z-index','2147483647','important');panel.style.setProperty('left',b.left+'px','important');panel.style.setProperty('top',b.top+'px','important');panel.style.setProperty('right',b.right+'px','important');panel.style.setProperty('bottom',b.bottom+'px','important');}
function findDiscordSettingsCloseButton(){return document.querySelector('[class*="standardSidebarView"] [class*="toolsContainer"] [class*="closeButton"], [class*="standardSidebarView"] [class*="toolsContainer"] [role="button"], [class*="standardSidebarView"] [data-testid="settings-close-button"], [class*="standardSidebarView"] [aria-label="Close"], [class*="toolsContainer"] [class*="closeButton"], [class*="toolsContainer"] [role="button"]');}
function closeSettings(){var discordClose=findDiscordSettingsCloseButton();if(discordClose&&discordClose.isConnected){discordClose.click();dispose();return;}var d=getFluxDispatcher();if(d){try{d.dispatch({type:'USER_SETTINGS_MODAL_CLOSE'});}catch(e){}try{d.dispatch({type:'LAYER_POP'});}catch(e){}}dispose();}
function normId(p){return (p||'').trim().replace(/^["']+|["']+$/g,'').replace(/\//g,'\\').toLowerCase();}
function isGameDisabled(disabledMap,p){if(!disabledMap||!p)return false;var norm=normId(p),raw=(p||'').toLowerCase();return !!(disabledMap[norm]||disabledMap[raw]);}
...
function render(data){var panel=document.getElementById(panelId);if(!panel)return;panel.innerHTML='';var closeBtn=document.createElement('div');closeBtn.className='vc-close-button';closeBtn.title='Close (ESC)';closeBtn.setAttribute('role','button');closeBtn.setAttribute('aria-label','Close');closeBtn.tabIndex=0;closeBtn.innerHTML='<svg width="18" height="18" viewBox="0 0 24 24"><path fill="currentColor" d="M18.4 4L12 10.4L5.6 4L4 5.6L10.4 12L4 18.4L5.6 20L12 13.6L18.4 20L20 18.4L13.6 12L20 5.6L18.4 4Z"/></svg><div class="vc-keybind">ESC</div>';closeBtn.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();closeSettings();});closeBtn.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();e.stopPropagation();closeSettings();}});panel.append(closeBtn);var heading=document.createElement('h1');heading.textContent='Registered Games';var intro=document.createElement('p');intro.textContent='DiscordLite watches visible Windows applications and can show them as your activity. Add an executable manually when detection does not find it.';var actions=document.createElement('div');actions.className='vc-game-actions';var pathInput=document.createElement('input');pathInput.type='text';pathInput.placeholder='Executable path (optional)';pathInput.dataset.vcGamePath='1';actions.append(pathInput,button('Browse','add','vc-primary'),button('Add','add-path'),button('Refresh','refresh'));var toggleLabel=document.createElement('label');toggleLabel.className='vc-game-toggle';var switchLabel=document.createElement('span');switchLabel.className='vc-switch';var toggle=document.createElement('input');toggle.type='checkbox';toggle.checked=!!data.detectionEnabled;toggle.dataset.vcDetectionToggle='1';var slider=document.createElement('span');slider.className='vc-slider';switchLabel.append(toggle,slider);toggleLabel.append(switchLabel,text('Enable game detection'));panel.append(heading,intro,actions,toggleLabel);var currentHeading=document.createElement('h2');currentHeading.textContent='Running now';panel.append(currentHeading);var running=data.runningGames||[];if(!running.length){var empty=document.createElement('div');empty.className='vc-game-empty';empty.textContent=data.detectionEnabled?'No visible games detected.':'Game detection is disabled.';panel.append(empty);}else{running.forEach(function(game){var isEnabled=!isGameDisabled(data.disabledGames,game.path);panel.append(row(resolveDisplayName(game.name,game.path,data.overrides),game.windowTitle,game.path,isEnabled,false));});}var seenHeading=document.createElement('h2');seenHeading.textContent='Games seen';panel.append(seenHeading);var seen=data.gamesSeen||[];if(!seen.length){var emptySeen=document.createElement('div');emptySeen.className='vc-game-empty';emptySeen.textContent='No games have been registered yet.';panel.append(emptySeen);}else{seen.forEach(function(game){var isEnabled=!isGameDisabled(data.disabledGames,game.path);panel.append(row(resolveDisplayName(game.name,game.path,data.overrides),game.source==='manual'?'Added manually':'Detected',game.path,isEnabled,true));});}}
...
function open(){active=true;var item=document.getElementById(navId),panel=document.getElementById(panelId);if(!item){mount();item=document.getElementById(navId);panel=document.getElementById(panelId);}if(item){var otherSelected=document.querySelector('[class*="item_"][class*="selected_"]'),selClass=otherSelected?otherSelected.className.match(/\bselected_\w+\b/)?.[0]:null;if(otherSelected&&selClass)otherSelected.classList.remove(selClass);item.classList.add('vc-selected');if(selClass)item.classList.add(selClass);}if(!panel)panel=installPanel();if(panel){panel.style.setProperty('display','block','important');panel.style.setProperty('visibility','visible','important');panel.style.setProperty('opacity','1','important');panel.style.setProperty('pointer-events','auto','important');position(panel);var currentData=lastAppliedData||window.__vcGameActivityInitial;if(currentData)render(currentData);refresh();}}
function installPanel(){var panel=document.getElementById(panelId);if(!panel){panel=document.createElement('div');panel.id=panelId;panel.style.setProperty('display','none','important');document.body.appendChild(panel);}position(panel);return panel;}
...
document.addEventListener('change',function(event){var action=event.target.closest&&event.target.closest('[data-vc-detection-toggle]');if(action){invoke('vc_game_activity_set_detection',{enabled:!!action.checked}).then(render).catch(function(error){console.error(error);});return;}var gameToggle=event.target.closest&&event.target.closest('[data-vc-game-toggle]');if(gameToggle){var path=gameToggle.dataset.vcPath;if(path)invoke('vc_game_activity_toggle_game',{path:path,enabled:!!gameToggle.checked}).then(render).catch(showActionError);}},true);
window.addEventListener('keydown',function(event){if(event.key==='Escape'&&active&&isSettingsOpen()){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();closeSettings();}},true);
window.addEventListener('resize',function(){var panel=document.getElementById(panelId);if(panel&&panel.style.display!=='none')position(panel);});
...
setInterval(function(){if(active&&!loading&&isSettingsOpen()){var panel=document.getElementById(panelId);if(panel&&panel.style.display!=='none')position(panel);refresh();}flushPendingPresence();},1000);
