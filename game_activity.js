...
function closeSettings(){var panel=document.getElementById(panelId);var discordClose=document.querySelector('[aria-label="Close"], [class*="closeButton"]');if(discordClose&&discordClose.isConnected&&(!panel||!panel.contains(discordClose))){discordClose.click();return;}window.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',code:'Escape',keyCode:27,which:27,bubbles:true}));document.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',code:'Escape',keyCode:27,which:27,bubbles:true}));setTimeout(function(){if(!isSettingsOpen())dispose();else close();},50);}
