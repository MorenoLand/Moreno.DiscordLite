...
function open(){active=true;var item=document.getElementById(navId),panel=document.getElementById(panelId);if(!item){mount();item=document.getElementById(navId);panel=document.getElementById(panelId);}if(item){var otherSelected=document.querySelector('[class*="item_"][class*="selected_"]'),selClass=otherSelected?otherSelected.className.match(/\bselected_\w+\b/)?.[0]:null;if(otherSelected&&selClass)otherSelected.classList.remove(selClass);item.classList.add('vc-selected');if(selClass)item.classList.add(selClass);}if(panel){panel.style.setProperty('display','block','important');panel.style.setProperty('visibility','visible','important');panel.style.setProperty('opacity','1','important');panel.style.setProperty('pointer-events','auto','important');position(panel,panel.__vcModal);var currentData=lastAppliedData||window.__vcGameActivityInitial;if(currentData)render(currentData);refresh();}}
...
function mount(){addStyles();var label=findExact(document,'Activity Privacy');if(!label)return;var refContent=label.closest('[class*="itemContent"]')||label.parentElement;var refItem=label.closest('[role="link"],[role="button"],[role="tab"],a,button,[class*="item_"]')||refContent.parentElement;var refLi=refItem.closest('li,[class*="itemContainer"]')||refItem.parentElement;var container=refLi.parentElement;if(!container)return;var connected=findExact(document,'Connected Apps');var connectedLi=connected?(connected.closest('li,[class*="itemContainer"]')||connected.parentElement):null;var before=(connectedLi&&connectedLi.parentElement===container)?connectedLi:refLi.nextSibling;var modal=findModal(label);var panel=installPanel(modal);var li=document.getElementById('vc-game-activity-li');var item=document.getElementById(navId);if(!li||!item||li.parentElement!==container){if(li)li.remove();if(item&&!li)item.remove();li=document.createElement(refLi.tagName.toLowerCase());li.id='vc-game-activity-li';if(refLi.className)li.className=refLi.className;li.setAttribute('data-settings-sidebar-item','registered_games_panel');item=document.createElement('div');item.id=navId;if(refItem.className)item.className=refItem.className;item.setAttribute('role',refItem.getAttribute('role')||'link');item.tabIndex=0;item.setAttribute('data-list-item-id','settings-sidebar___registered_games_panel');var content=document.createElement('div');if(refContent&&refContent.className)content.className=refContent.className;var refIcon=(refContent&&refContent.querySelector('svg,[class*="icon"]'))||refItem.querySelector('svg,[class*="icon"]');if(refIcon){var icon=refIcon.cloneNode(true);icon.classList.add('vc-game-activity-icon');content.appendChild(icon);}var itemLabel=document.createElement('div');if(label.className)itemLabel.className=label.className;if(label.dataset.textVariant)itemLabel.dataset.textVariant=label.dataset.textVariant;itemLabel.style.color='currentColor';itemLabel.textContent='Registered Games';content.appendChild(itemLabel);item.appendChild(content);li.appendChild(item);item.addEventListener('click',function(event){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();open();});item.addEventListener('keydown',function(event){if(event.key==='Enter'||event.key===' '){event.preventDefault();event.stopPropagation();open();}});if(before)container.insertBefore(li,before);else container.appendChild(li);}else{if(refLi.className&&li.className!==refLi.className)li.className=refLi.className;if(refItem.className&&!active&&item.className!==refItem.className)item.className=refItem.className;}if(active){item.classList.add('vc-selected');panel.style.display='block';position(panel,modal);refresh();}}
...
var observer=new MutationObserver(function(){if(!findContentPane()){if(document.getElementById(panelId)||document.getElementById(navId)||document.getElementById('vc-game-activity-li'))dispose();return;}mount();});
